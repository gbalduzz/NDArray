...
auto getBroadcastShape(const std::array<std::size_t, I>&... shapes) {
  ...
  bool broadcast = !pack_equal<I...>;

  auto check_shape = [&](const auto& to_check) {
    for (unsigned i = 0; i < to_check.size(); ++i) {
      auto& shape_val = result_shape[result_shape.size() - 1 - i];
      const auto check_val = to_check[to_check.size() - 1 - i];

      if (shape_val == 1) {
        shape_val = check_val;
      }
      else {
        if (shape_val != check_val) {
          if (check_val != 1)
            throw(std::invalid_argument("Could not broadcast tensors with different shapes."));
          else
            broadcast = true;
        }
      }
  ...
  (..., check_shape(shapes));
  return std::make_pair(result_shape, broadcast);
}
